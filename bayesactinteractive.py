...
def bayesactinteractive(argv, plotter=None,verbose=False):
    ...
    while not done:
        ...
        if learn_turn=="client" and learn_observ==[]:
            ...
            learn_avgs=learn_agent.propagate_forward(learn_aab,learn_observ,learn_xobserv,learn_paab,verb=learn_verbose,plotter=plotter,agent=eTurn.learner)
...
def main(argv):
    ...
    if (False == plot):
        bayesactinteractive(argv,None)
    else:
        plotter = cBayesactSimGui()
        bayesactSimThread = threading.Thread(target=bayesactinteractive, args=(argv,plotter))
